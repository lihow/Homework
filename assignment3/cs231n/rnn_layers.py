...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    prev_h = h0
    for i in range(T):
        h[:, i, :], cache_ = rnn_step_forward(x[:, i, :], prev_h, Wx, Wh, b)
        cache.append(cache_)
        prev_h = h[:, i, :]
    ##############################################################################
    #                               END OF YOUR CODE                             #
...
def rnn_backward(dh, cache):
    ...
    ##############################################################################
    N, T, H = dh.shape
    x, _, Wx, Wh, b, back_h = cache[0]#x, prev_h, Wx, Wh, b, next_h
    _, D = x.shape
    dx = np.zeros((N, T, D))
    dWx = np.zeros_like(Wx)
    dWh = np.zeros_like(Wh)
    db = np.zeros_like(b)
    dback_h = np.zeros_like(back_h)
    for i in range(T-1, -1, -1):#倒序
        dnext_h = dh[:, i, :] + dback_h
        dx[:, i, :], dback_h, dWxt, dWht, dbt = rnn_step_backward(dnext_h, cache[i])
        dWx += dWxt
        dWh += dWht
        db += dbt
        dh0 = dback_h
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    di = (dnext_c + dh2c) * g * i * (1 - i)
    df = (dnext_c + dh2c) * prev_c * f * (1 - f)
    dg = (dnext_c + dh2c) * i * (1 - g**2)
    dprev_c = (dnext_c + dh2c) * f
