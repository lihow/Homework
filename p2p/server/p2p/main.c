...
int main(int argc, char *argv[])
{
	...
	//创建数据库 filename, filehash, f_size, peeraddr
	//const char *sql = "CREATE TABLE IF NOT EXISTS files(filename VARCHAR(128),"  
    //    "filehash CHAR(50),"
    //    "f_size INTEGER"     		
    //    "peeraddr CHAR(50)"  
    //    ");";
    ...
    //sprintf(query, "DELETE FROM '%s'", DB_FILE);
    //printf("%s", query);
...
void *p2p(void *args)
{
	...
	while(strcmp(in, "QUIT") != 0)
	{
		...
		if(strncmp(in, "ADD", 3) == 0)//比较特定长度的字符串 前3个字符相等返回0
		{
			//printf("add 文件");
			strtok(in, " ");
			...
			if(filename != NULL)
			{
				...
				if(filehash != NULL)
				{
					...
					if((filesize != NULL) && (validate_int(filesize) == 1))
					{
						...
						sprintf(query, "INSERT INTO files(filename, filehash, f_size, peeraddr) VALUES('%s', '%s', '%ld', '%s')", filename, filehash, f_size, peeraddr);
						printf(query);
		...
		else if(strcmp(in, "LIST") == 0)
		{
			...
			while((status = sqlite3_step(stmt)) != SQLITE_DONE)
			{
				...
				else if(strcmp(peeraddr,(char *) sqlite3_column_text(stmt, 2)) == 0)
				{			
